...
type RAGIndexer struct {
	embedding embedding.Embedder    // embedding 模型实例，用于将文本转换为向量
	indexer   *redisIndexer.Indexer // Redis 索引器实例，负责将文档和其转换成的向量存储到 Redis 中
}

type RAGQuery struct {
	embedding embedding.Embedder  // embedding 模型实例，用于将原始问题转换为向量
	retriever retriever.Retriever // Redis 检索器实例，根据原始问题转换成的向量从 Redis 中检索相关文档
}
