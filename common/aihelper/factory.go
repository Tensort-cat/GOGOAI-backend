...
func (f *AIModelFactory) registerCreators() {
	...
	// MCP 模型（集成MCP服务）
	f.creators["3"] = func(ctx context.Context, config map[string]any) (AIModel, error) {
		username, ok := config["username"].(string)
		if !ok {
	...
	f.creators["4"] = func(ctx context.Context, config map[string]any) (AIModel, error) {
...
func (f *AIModelFactory) CreateAIHelper(ctx context.Context, modelType string, SessionID string, config map[string]any) (*AIHelper, error) {
